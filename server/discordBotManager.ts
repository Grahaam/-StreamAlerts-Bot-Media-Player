			...
			this.client.on("messageCreate", async (message: Message) => {
				try {
					...
					if (cooldown > 0) {
						...
						if (diff < cooldown) {
							console.warn(
								`[Discord] Cooldown active for ${message.author.username}`,
							);
							logManager.addLog({
					...
					if (textCheck.wasBlocked) {
						console.warn(
							`[Discord] Message from ${message.author.username} blocked (banned word)`,
						);
						logManager.addLog({
							author: message.author.username,
							text: message.content,
					...
					if (settingsManager.settings.blockLinks) {
						...
						if (remainingMatches.length > 0) {
							console.warn(
								`[Discord] Message from ${message.author.username} blocked (extra links)`,
							);
							logManager.addLog({
								author: message.author.username,
								text: message.content,
					...
					if (settingsManager.settings.blockNSFW) {
						const hasSpoilerAttachment = message.attachments.some(
							(a) => a.spoiler,
						);
						...
						if (hasSpoilerAttachment || hasNSFWText) {
							console.warn(
								`[Discord] Message from ${message.author.username} blocked (NSFW/Spoiler)`,
							);
							logManager.addLog({
								author: message.author.username,
								text: message.content,
					...
					const alertPayload = {
						...
						stopAlertShortcut:
							settingsManager.settings.stopAlertShortcut || "Escape",
						timestamp: Date.now(),
					};
					...
					logManager.addLog({
						...
						reason: alertPayload.ytDlpError
							? `Iframe fallback (yt-dlp error: ${alertPayload.ytDlpError.substring(0, 50)}...)`
