  ...
  useEffect(() => {
    ...
    const runNextAlert = async () => {
      ...
      if (nextItem.type === "video") {
        ...
        // For video alerts (HTML5 and YouTube), we ignore the default static parameter
                ...
                <>
                  {!embedMode && isVertical ? (
                    <div 
                      ...
                      style={{ 
                        width: '120%', 
                        height: '120%', 
                        backgroundImage: `url(${activeAlert.mediaUrl})`, // This is purely a fallback if image, but we are a video
                        filter: 'blur(20px)',
                  ...
                  <video
                    ...
                    onError={(e) => {
                      ...
                      const msg = `[Video Debug] onError - src: ${rawUrl.substring(0, 150)}... Code: ${err?.code}, Message: ${err?.message}, networkState: ${e.currentTarget.networkState}, readyState: ${e.currentTarget.readyState}`;
                      console.error(msg);
                      
                      if (err?.code === 4 && activeAlert && !activeAlert.mediaUrl.includes("retry=1")) {
                        console.log("[Video Debug] Automatically retrying video stream loading after short delay...");
                        setTimeout(() => {
                        ...
                        console.log("[Video Debug] Proxy media retry failed, attempting direct load fallback...");
                        ...
                        if (rawUrlMatch && rawUrlMatch[1]) {
                           ...
                           console.log(`[Video Debug] Extracting raw URL for fallback: ${fallbackUrl.substring(0, 100)}...`);
                           setTimeout(() => {
                        ...
                        console.log("[Video Debug] Exhausted retry attempts. Stopping playback.");
                        onVideoErrorRef.current?.();
                      }
                    }}
                    onPlay={(e) => {
