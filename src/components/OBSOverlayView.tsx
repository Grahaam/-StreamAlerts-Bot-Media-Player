		...
		queue.forEach((item) => {
			...
			if (item.type === "iframe" || item.type === "link") {
				...
				video.onerror = (e: any) => {
					...
					console.error(
						`Preload error for ${item.mediaUrl.substring(0, 50)}...`,
						target.error,
					);
					setPreloadedUrls((prev) => ({ ...prev, [item.mediaUrl]: true }));
				};
										...
										>
											{activeAlert.type === "video" ? (
												<>
													...
													<video
														...
														onError={(e) => {
															...
															console.error(
																`Video error for ${rawUrl.substring(0, 50)}...`,
																err?.message,
															);
															...
															if (
																err?.code === 4 &&
																activeAlert &&
																!activeAlert.mediaUrl.includes("retry=1")
															) {
																...
																!activeAlert.mediaUrl.includes("fallback=1")
															) {
																console.log("Proxy failed, trying direct link");
																const rawUrlMatch =
																	activeAlert.mediaUrl.match(/[?&]url=([^&]+)/);
														...
														onPause={() => {
															if (
																cancelCurrentAlertRef.current &&
																extendCurrentTimeoutRef.current
															) {
														...
														onLoadedMetadata={(e) => {
															const videoDurationMs =
																e.currentTarget.duration * 1000;
															console.log(
																`Video metadata: ${videoDurationMs}ms`,
															);
															if (
																videoDurationMs &&
																!isNaN(videoDurationMs) &&
																isFinite(videoDurationMs)
															) {
																onVideoLoadedMetadataRef.current?.(
																	videoDurationMs,
												...
												<div
													...
													onMouseEnter={() => {
														if (extendCurrentTimeoutRef.current)
															extendCurrentTimeoutRef.current(3600000);
													}}
												>
													<iframe
													...
													{activeAlert.provider === "instagram" && (
														<div className="absolute top-4 left-4 right-4 bg-red-600/90 text-white text-sm p-3 rounded-xl shadow-lg z-20 backdrop-blur-md border border-red-500 animate-in fade-in slide-in-from-top-4 pointer-events-none">
															<p className="font-bold mb-1">
																[Warning] Autoplay Blocked by Instagram
															</p>
															<p className="opacity-90 leading-tight">
																Instagram blocks automated video downloads
																without a login. To enable native MP4 autoplay
																for Instagram reels, please upload a{" "}
																<code>cookies.txt</code> file to the root
																directory.
															</p>
														</div>
													)}
													{activeAlert.provider === "tiktok" && (
														<div className="absolute top-4 left-4 right-4 bg-orange-600/90 text-white text-sm p-3 rounded-xl shadow-lg z-20 backdrop-blur-md border border-orange-500 animate-in fade-in slide-in-from-top-4 pointer-events-none">
															<p className="font-bold mb-1">
																[Warning] TikTok Page Embed
															</p>
															<p className="opacity-90 leading-tight">
																Direct video extraction failed. Showing standard
																browser embedded player which may require
																clicking to play.
															</p>
														</div>
													)}
			...
			{embedMode && !activeAlert && queue.length === 0 && (
				<div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 text-slate-500">
					...
					<span className="text-sm font-medium">
						Real-time Preview: Overlay Idle
					</span>
					<span className="text-xs text-slate-600 mt-1">
						Trigger a test simulation below
					</span>
				</div>
			)}
		</div>
